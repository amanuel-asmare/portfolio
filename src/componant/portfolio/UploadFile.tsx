                  ...
                  >
                    ...
                    <td className="px-4 py-2 flex space-x-3">
                      {isOpenable(file.mimeType) && (
                        <a
                          href={`${import.meta.env.VITE_API_URL}/uploads/${encodeURIComponent(file.filename)}`}
                          target="_blank"
...
const UploadFile: React.FC = () => {
  ...
  const handleDownload = async (filename: string, originalName: string) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/uploads/${encodeURIComponent(filename)}`, {
        responseType: 'blob',
      });
